...
def fix_all_data_quality_issues():
    """Fix all data quality issues found in the analysis"""
    ...
    # Get comprehensive player analysis with all fixes
    comprehensive_query = """
    ...
    # Check for specific players we know had issues
    print("Checking for previously problematic players:")
    
    # Check Pastrnak
    pastrnak = player_results[player_results['player_name'].str.contains('Pastrnak', na=False)]
    if not pastrnak.empty:
        print(f"✅ Pastrnak: {pastrnak['performance_tier'].iloc[0]} (Pts/60: {pastrnak['points_60'].iloc[0]:.1f})")
    else:
        print("❌ Pastrnak not found")
    
    # Check Ryan Donato
    donato = player_results[player_results['player_name'].str.contains('Donato', na=False)]
    if not donato.empty:
        print(f"✅ Ryan Donato: {donato['performance_tier'].iloc[0]} (Pts/60: {donato['points_60'].iloc[0]:.1f})")
    else:
        print("❌ Ryan Donato not found")
    
    # Check for duplicates
    duplicate_check = player_results.groupby(['team_abbr', 'player_name']).size()
    duplicates = duplicate_check[duplicate_check > 1]
    if not duplicates.empty:
        print(f"❌ Found {len(duplicates)} duplicate players:")
        for (team, player), count in duplicates.items():
            print(f"  - {player} ({team}): {count} entries")
    else:
    ...
    with open('/Users/markhenderson/Cursor Projects/NHL-API/projected_rosters_2025_26.md', 'w') as f:
        f.write('# 2025-26 Projected NHL Rosters - Final Analysis (All Issues Fixed)\n\n')
        f.write('**FINAL VERSION** - All data quality issues resolved\n\n')
        f.write('## Data Quality Fixes Applied:\n')
        f.write('- ✅ Fixed missing points data (Pastrnak, Ryan Donato)\n')
        f.write('- ✅ Removed duplicate entries (Martin Necas, Mikko Rantanen, Zach Werenski, Cam Fowler)\n')
        f.write('- ✅ Handled high Pts/60 but low total points (Tyler Seguin, etc.)\n')
        f.write('- ✅ Proper deduplication using ROW_NUMBER()\n\n')
    ...
    print("✅ Fixed all data quality issues!")
