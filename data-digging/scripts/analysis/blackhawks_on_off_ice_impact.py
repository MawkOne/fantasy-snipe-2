#!/usr/bin/env python3
"""
Chicago Blackhawks On-Ice vs Off-Ice Impact Analysis - 2024-25 Season
Analyze each Blackhawk's relative impact when on ice vs off ice - 5v5 only
"""
...
def get_blackhawks_2024_25_data() -> pd.DataFrame:
    """
    Get 2024-25 season data for Chicago Blackhawks players
    """
    client = bigquery.Client()
    
    # Query to get 2024-25 season data for Blackhawks players - 5v5 only
    query = """
    SELECT 
        ps.full_name,
    ...
    AND ps.position != 'G'
    AND ps.team_abbrev = 'CHI'
    ORDER BY ps.ev_points DESC
    """
    
    return client.query(query).to_dataframe()

def calculate_blackhawks_team_totals_and_relative_impact(df: pd.DataFrame) -> pd.DataFrame:
    """
    Calculate Blackhawks team totals and each player's relative contribution
    """
    df = df.copy()
    
    # Calculate Blackhawks team totals
    team_totals = df.agg({
        'points': 'sum',
        'goals': 'sum',
        'assists': 'sum',
...
def classify_blackhawks_players(df: pd.DataFrame) -> pd.DataFrame:
    """
    Classify Blackhawks players into tiers
    """
    df = df.copy()
...
def create_blackhawks_on_off_ice_visualization(df: pd.DataFrame) -> None:
    """
    Create visualization for Blackhawks on-ice vs off-ice impact
    """
    # Create the plot
    ...
    ax1.set_title('Chicago Blackhawks: On-Ice vs Off-Ice Points Impact (5v5 Only)\\n2024-25 Season')
    ax1.set_xticks(x)
    ...
    ax2.set_title('Chicago Blackhawks: On-Off Ice Impact Score (5v5 Only)\\n2024-25 Season')
    ...
    ax3.set_title('Chicago Blackhawks: Relative Team Impact Score (5v5 Only)\\n2024-25 Season')
    ...
    ax4.set_title('Chicago Blackhawks: Points/60 vs TOI per Game (5v5 Only)\\n2024-25 Season')
...
def create_detailed_blackhawks_analysis_table(df: pd.DataFrame) -> None:
    """
    Create detailed table of Blackhawks on-ice vs off-ice analysis
    """
    print("\\n" + "="*150)
    print("CHICAGO BLACKHAWKS ON-ICE vs OFF-ICE IMPACT ANALYSIS - 2024-25 SEASON (5v5 ONLY)")
    print("="*150)
    ...
    # Key insights
    print("\\n" + "="*150)
    print("KEY INSIGHTS - CHICAGO BLACKHAWKS")
    print("="*150)
    ...
    # Team totals
    print(f"\\n📊 CHICAGO BLACKHAWKS TEAM TOTALS - 2024-25 (5v5 ONLY):")
...
def main():
    """
    Main function to run the Blackhawks on-ice vs off-ice impact analysis
    """
    print("CHICAGO BLACKHAWKS ON-ICE vs OFF-ICE IMPACT ANALYSIS - 2024-25 SEASON (5v5 ONLY)")
    print("Analyzing each Blackhawk's relative impact when on ice vs off ice - Even Strength Only")
    print("="*80)
    
    # Get Blackhawks 2024-25 player data
    print("\\nFetching Chicago Blackhawks 2024-25 season data...")
    try:
        blackhawks_df = get_blackhawks_2024_25_data()
        print(f"✅ Found data for {len(blackhawks_df)} Chicago Blackhawks players in 2024-25 season")
    except Exception as e:
        print(f"❌ Error fetching Blackhawks data: {e}")
        return
    
    # Calculate team totals and relative impact
    print("\\nCalculating team totals and relative impact...")
    df_with_team_totals = calculate_blackhawks_team_totals_and_relative_impact(blackhawks_df)
    
    # Classify players
    print("\\nClassifying Blackhawks players into tiers...")
    classified_df = classify_blackhawks_players(df_with_team_totals)
    
    # Create visualizations
    print("\\nCreating Blackhawks on-ice vs off-ice impact visualizations...")
    create_blackhawks_on_off_ice_visualization(classified_df)
    
    # Create detailed analysis
    create_detailed_blackhawks_analysis_table(classified_df)
    
    print("\\n" + "="*80)
    print("CHICAGO BLACKHAWKS ANALYSIS COMPLETE")
    print("="*80)
    print("\\nThis analysis shows each Blackhawk's impact when they are on the ice versus when they are off the ice (5v5 only).")
    ...
    print("- This metric helps identify the most important players to the Blackhawks' success")
