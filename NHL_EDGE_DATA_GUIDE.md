...
### Quick Start Scripts
We have provided scripts in the `nhl_edge/` directory to automate this process.

1.  **List Goals & Animation URLs for a Game:**
    ```bash
    python nhl_edge/list_game_goals.py <GAME_ID>
    ```
    *Example: `python nhl_edge/list_game_goals.py 2025020360`*

2.  **Fetch Animation Data:**
    ```bash
    python nhl_edge/fetch_animation.py <URL> <OUTPUT_FILE.json>
    ```
    *Use the URL from step 1.*

3.  **Generate Visualization:**
    ```bash
    python nhl_edge/generate_viz.py <OUTPUT_FILE.json> goal_viz.html
    ```
    *Open `goal_viz.html` in your browser to see the animation with live speed tracking.*
