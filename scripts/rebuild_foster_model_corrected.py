#!/usr/bin/env python3
"""
Rebuild the entire Foster model with corrected individual stats.
This creates a complete, working Foster forecasting model with realistic projections.
"""
...
def rebuild_foster_model_corrected():
    """Rebuild the entire Foster model with corrected individual stats."""
    client = bigquery.Client()
    
    print("Rebuilding Foster model with corrected individual stats...")
    
    # 1. Create corrected line assignments
    print("1. Creating corrected line assignments...")
    line_assignments_query = """
    CREATE OR REPLACE TABLE `fantasy-snipe-ai.nhl_projections.line_assignments_corrected` AS
    ...
    line_assignments AS (
        SELECT 
            ...
            'Foster Model v1.3 (Final)' as model_version
    ...
    job.result()
    print("✓ Created corrected line assignments")
    
    # 2. Create corrected line forecasts
    print("2. Creating corrected line forecasts...")
    line_forecasts_query = """
    CREATE OR REPLACE TABLE `fantasy-snipe-ai.nhl_projections.line_forecasts_corrected` AS
    WITH team_lines AS (
        SELECT 
            la.team,
            la.primary_role,
        ...
        FROM `fantasy-snipe-ai.nhl_projections.line_assignments_corrected` la
    ...
    job.result()
    print("✓ Created corrected line forecasts")
    
    # 3. Create corrected player forecasts
    print("3. Creating corrected player forecasts...")
    ...
    WITH player_line_forecasts AS (
        ...
        FROM `fantasy-snipe-ai.nhl_projections.line_assignments_corrected` la
        LEFT JOIN `fantasy-snipe-ai.nhl_projections.line_forecasts_corrected` lf 
    ...
    job.result()
    print("✓ Created corrected player forecasts")
    
    # 4. Create final corrected view
    print("4. Creating final corrected view...")
    final_view_query = """
    CREATE OR REPLACE VIEW `fantasy-snipe-ai.nhl_projections.current_player_forecasts_final_corrected` AS
    SELECT 
        pf.player_id,
        pf.player_name,
        pf.position,
    ...
    print("\nRunning comprehensive QA on corrected Foster model...")
    ...
    FROM `fantasy-snipe-ai.nhl_projections.line_forecasts_corrected`
    WHERE team = 'EDM'
    ORDER BY line_role
    """
    ...
    FROM `fantasy-snipe-ai.nhl_projections.current_player_forecasts_final_corrected`
    WHERE team = 'EDM'
...
if __name__ == "__main__":
    rebuild_foster_model_corrected()
