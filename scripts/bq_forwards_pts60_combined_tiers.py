...
def create_forwards_total_pts_age_curves(df: pd.DataFrame) -> None:
    ...
    ax2.set_title('Center vs Wing PTS/60 by Age')
    ax2.legend()
    ...
    ax3.set_title('Elite & High Forwards PTS/60 by Position')
    ax3.legend()
...
def analyze_forwards_pts60_peak_ages(df: pd.DataFrame) -> Dict:
    """Analyze peak PTS/60 ages by combined tier and position for forwards."""
...
def main():
    parser = argparse.ArgumentParser(description='Analyze Forwards PTS/60 age curves with combined tiers')
    parser.add_argument('--min-games', type=int, default=20, help='Minimum games played per season')
    parser.add_argument('--save-plots', action='store_true', help='Save plots to files')
    args = parser.parse_args()
    
    print("Loading Forwards PTS/60 age data with combined tiers...")
    ...
    print("\nPTS/60 Statistics by Position (Center vs Wing):")
    pos_stats = df.groupby('position_group')['avg_pts60'].agg(['count', 'mean', 'std', 'min', 'max']).round(2)
    print(pos_stats)
    
    # Create PTS/60 age curves
    print("\nCreating Forwards PTS/60 age curves with combined tiers...")
    create_forwards_pts60_age_curves(df)
    
    if args.save_plots:
        plt.savefig('forwards_pts60_combined_tiers.png', dpi=300, bbox_inches='tight')
        print("Saved plot: forwards_pts60_combined_tiers.png")
    ...
    print("\nPeak PTS/60 Age Analysis (Forwards - All Tiers):")
