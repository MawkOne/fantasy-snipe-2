#!/usr/bin/env python3
"""
Fixed Hidden Gems Analysis - 2024-25 Season
Properly excluding elite players to find true hidden gems
"""
...
def calculate_team_averages_2024_25(df: pd.DataFrame) -> pd.DataFrame:
    ...
    team_averages.columns = ['team_avg_pts60', 'team_avg_toi', 'team_avg_shooting', 'team_avg_points']
...
def create_hidden_gems_visualization_2024_25(df: pd.DataFrame) -> None:
    ...
    top_50 = df.head(50)
    players = top_50['full_name']
    impact = top_50['impact_vs_team_pts60']
    tiers = top_50['tier']
...
def show_elite_players_excluded(df: pd.DataFrame) -> None:
    """
    Show which elite players were excluded from the analysis
    """
    elite_players = df[df['tier'] == 'Elite'].copy()
    
    print("\\n" + "="*80)
    print("ELITE PLAYERS EXCLUDED FROM HIDDEN GEMS ANALYSIS")
    print("="*80)
    ...
    print(f"\\nTotal Elite Players Excluded: {len(elite_players)}")
...
def main():
    ...
    # Classify players properly
    print("\\nClassifying players into tiers...")
    classified_df = classify_players_properly(all_players_df)
    
    # Show elite players that will be excluded
    show_elite_players_excluded(classified_df)
    ...
    hidden_gems_df = find_hidden_gems_2024_25(df_with_team_avg, top_n=50)
