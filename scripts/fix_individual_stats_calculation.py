#!/usr/bin/env python3
"""
Fix the individual stats calculation to use proper individual player stats instead of on-ice metrics.
This corrects the points/60, goals/60, assists/60 calculations to realistic values.
"""
...
def fix_individual_stats_calculation():
    """Fix individual stats calculation to use proper individual player stats."""
    client = bigquery.Client()
    
    print("Fixing individual stats calculation...")
    
    # Create corrected player input templates using individual stats
    fix_templates_query = """
    CREATE OR REPLACE TABLE `fantasy-snipe-ai.nhl_projections.player_input_templates_corrected` AS
    WITH player_3yr_stats AS (
        -- Get 3-year averages for each player using INDIVIDUAL stats
        SELECT 
            ps.player_id,
            ps.full_name,
            ps.position,
    ...
    player_3yr_averages AS (
        ...
        SELECT 
            ...
            -- Use CORRECTED individual stats from NHL Stats API
    ...
    SELECT 
        ...
        -- Points conversion (Foster's requirement) - CORRECTED
        ROUND(p3a.points_60_3yr_avg / NULLIF(pev.ev_cf60_3yr_avg, 0), 4) as ev_pts_conversion,
        
        -- Individual scoring stats (CORRECTED - using individual stats)
        ...
        'Foster Model v1.2 (Corrected)' as model_version
    ...
    job.result()
    print("✓ Created corrected player input templates")
    
    # Run QA on corrected templates
    print("\nRunning QA on corrected templates...")
    ...
    FROM `fantasy-snipe-ai.nhl_projections.player_input_templates_corrected`
    ...
    print("Corrected Connor McDavid Stats:")
    for row in qa_results:
        print(f"  Points/60: {row.ev_points_60}")
        print(f"  Goals/60: {row.ev_goals_60}")
        print(f"  Assists/60: {row.ev_assists_60}")
        print(f"  Points Conversion: {row.ev_pts_conversion}")
        print(f"  CF/60: {row.ev_cf60}")

if __name__ == "__main__":
    fix_individual_stats_calculation()
