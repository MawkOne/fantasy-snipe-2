#!/usr/bin/env python3
"""
Fix the Foster model FINALLY:
1. Ensure each player appears only once (no duplicates)
2. Use realistic TOI per player based on historical data
3. Fix the line assignment logic to be more accurate
"""
...
def fix_foster_model_final():
    """Fix the Foster model FINALLY with no duplicates and realistic TOI."""
    client = bigquery.Client()
    
    print("Fixing Foster model FINALLY...")
    
    # 1. Create final line assignments (one per player, realistic TOI)
    ...
    line_assignments AS (
        SELECT 
            ...
            'Foster Model v1.6 (Final)' as model_version
    ...
    job.result()
    print("✓ Created final line assignments (one per player, realistic TOI)")
    
    # 2. Create final line forecasts
    print("2. Creating final line forecasts...")
    line_forecasts_query = """
    CREATE OR REPLACE TABLE `fantasy-snipe-ai.nhl_projections.line_forecasts_final` AS
    WITH team_lines AS (
        SELECT 
            la.team,
            la.primary_role,
    ...
    job.result()
    print("✓ Created final line forecasts")
    
    # 3. Create final player forecasts
    print("3. Creating final player forecasts...")
    ...
    WITH player_line_forecasts AS (
        ...
        FROM `fantasy-snipe-ai.nhl_projections.line_assignments_final` la
        LEFT JOIN `fantasy-snipe-ai.nhl_projections.line_forecasts_final` lf 
    ...
    job.result()
    print("✓ Created final player forecasts")
    
    # 4. Create final view
    print("4. Creating final view...")
    final_view_query = """
    ...
    job.result()
    print("✓ Created final view")
    
    # Run comprehensive QA
    print("\nRunning comprehensive QA on final model...")
