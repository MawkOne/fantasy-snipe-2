#!/usr/bin/env python3
"""
True On-Ice vs Off-Ice Impact Analysis - Game Level - 2024-25 Season
...
def get_game_level_data(team_abbr: str) -> pd.DataFrame:
    """
    Get game-level data for a specific team to calculate true on-ice vs off-ice impact
    """
    client = bigquery.Client()
    
    # Query to get game-level data with team performance
    query = f"""
    WITH team_games AS (
        SELECT 
            g.id as game_id,
    ...
    SELECT 
        ...
        -- Calculate off-ice performance (team performance in games this player didn't play)
        (tt.avg_team_score - (pt.avg_team_score_with_player * pt.games_played / tt.total_games)) / 
        (1 - pt.games_played / tt.total_games) as avg_team_score_without_player,
        (tt.avg_opponent_score - (pt.avg_opponent_score_with_player * pt.games_played / tt.total_games)) / 
        (1 - pt.games_played / tt.total_games) as avg_opponent_score_without_player,
        (tt.avg_goal_differential - (pt.avg_goal_differential_with_player * pt.games_played / tt.total_games)) / 
        (1 - pt.games_played / tt.total_games) as avg_goal_differential_without_player
...
def calculate_true_impact_metrics(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df['team_score_impact'] = df['avg_team_score_with_player'] - df['avg_team_score_without_player']
    df['opponent_score_impact'] = df['avg_opponent_score_with_player'] - df['avg_opponent_score_without_player']
    df['goal_differential_impact'] = df['avg_goal_differential_with_player'] - df['avg_goal_differential_without_player']
...
def create_detailed_true_impact_table(df: pd.DataFrame, team_name: str) -> None:
    ...
    for _, row in df.iterrows():
        detailed_data.append({
            ...
            'Off-Ice GF/G': f"{row['avg_team_score_without_player']:.2f}",
            'GF Impact': f"{row['team_score_impact']:+.3f}",
            'On-Ice GA/G': f"{row['avg_opponent_score_with_player']:.2f}",
            'Off-Ice GA/G': f"{row['avg_opponent_score_without_player']:.2f}",
...
def analyze_team(team_abbr: str, team_name: str) -> None:
    ...
    impact_df = calculate_true_impact_metrics(game_df)
