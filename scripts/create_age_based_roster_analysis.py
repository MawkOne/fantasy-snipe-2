...
def create_age_based_roster_analysis():
    """Create age-based roster analysis with proper contention cycle logic"""
    
    client = bigquery.Client()
    
    print("="*80)
    print("CREATING AGE-BASED ROSTER ANALYSIS")
    print("="*80)
    
    # Get comprehensive player analysis with age-based logic
    comprehensive_query = """
    ...
    matched_players AS (
        SELECT 
            ...
            CASE 
                ...
                WHEN ps.position IN ("C", "L", "R") AND ps.points_60 >= pos.p80_points_60 AND ps.points >= pos.p80_total_points THEN "Good"
                WHEN ps.position = "D" AND ps.points_60 >= pos.p80_points_60 AND ps.points >= pos.p80_total_points THEN "Good"
            ...
            CASE 
                WHEN ps.current_age <= 25 AND ps.points_60 >= pos.p80_points_60 AND ps.points >= pos.p80_total_points THEN "Future Elite"
    ...
    with open('/Users/markhenderson/Cursor Projects/NHL-API/projected_rosters_2025_26.md', 'w') as f:
        f.write('# 2025-26 Projected NHL Rosters - Age-Based Analysis\n\n')
        f.write('**FINAL VERSION** - Age-based contention cycle with proper breakout candidate definition\n\n')
        ...
        f.write('- **Future Elite**: Age ≤25 + Top 20% Pts/60 + Top 20% total points\n')
        f.write('- **Near Elite**: Top 10% Pts/60 + Top 20% total points\n')
        f.write('- **Good**: Top 20% Pts/60 + Top 20% total points\n')
        ...
        f.write('## Contention Cycle Logic:\n')
        f.write('- **Rebuilding**: Future Elites + no elites + young core with low production\n')
        f.write('- **Window Coming**: Future Elites + Elites + young core with good production\n')
        f.write('- **Window Open**: Good side of age curve Elites + Core with good production\n')
        f.write('- **Win Now**: Elites at age curve peak + Core with good production\n')
        f.write('- **Window Closing**: Aging Elites + aging core with good production\n')
        f.write('- **Window Closed**: Aging Elites + young core\n\n')
    ...
    print("✅ Created age-based roster analysis!")
    ...
    # Show some key teams
    print("\nKey Teams by Cycle:")
    for cycle in ['Win Now', 'Window Open', 'Window Coming', 'Window Closing', 'Window Closed', 'Rebuilding']:
        cycle_teams = team_results[team_results['contention_cycle'] == cycle]
        if not cycle_teams.empty:
            print(f"\n{cycle}:")
            for _, team in cycle_teams.head(5).iterrows():
                print(f"  {team['team_abbr']}: {team['elite_players']} elite, {team['future_elites']} future elite, {team['core_players']} core")

if __name__ == "__main__":
    create_age_based_roster_analysis()
