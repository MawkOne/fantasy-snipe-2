...
def fix_elite_thresholds_and_qa_all_teams():
    """Fix elite player thresholds and QA all teams"""
    
    client = bigquery.Client()
    
    print("="*80)
    print("FIXING ELITE THRESHOLDS AND QA ALL TEAMS")
    print("="*80)
    
    # First, let's analyze the current percentile thresholds to set better ones
    print("Step 1: Analyzing current percentile thresholds...")
    ...
    SELECT 
        ...
        MAX(CASE WHEN points_60 >= APPROX_QUANTILES(points_60, 100)[OFFSET(95)] THEN points_60 END) as top_pts60,
        MAX(CASE WHEN points >= APPROX_QUANTILES(points, 100)[OFFSET(90)] THEN points END) as top_points
    ...
    thresholds = client.query(threshold_query).to_dataframe()
    
    print("Current Percentile Thresholds:")
    print("=" * 60)
    for _, row in thresholds.iterrows():
        print(f'{row.position:2} | Players: {row.total_players:3} | P95: {row.p95_points_60:.1f} | P90: {row.p90_points_60:.1f} | P80: {row.p80_points_60:.1f} | P90 Pts: {row.p90_total_points:.0f} | P80 Pts: {row.p80_total_points:.0f}')
    
    # Create corrected view with better thresholds
    print("\nStep 2: Creating corrected view with better thresholds...")
    ...
    matched_players AS (
        SELECT 
            ...
            CASE 
                ...
                ELSE "Aging"
            END as age_category,
            -- CORRECTED Performance tiers with better thresholds
            CASE 
                -- Forwards: Top 5% Pts/60 + Top 10% total points
                ...
                -- Good: Top 20% Pts/60 + Top 20% total points
    ...
    print("✅ Created corrected view with better elite thresholds")
...
if __name__ == "__main__":
    fix_elite_thresholds_and_qa_all_teams()
