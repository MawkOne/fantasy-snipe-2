...
def get_season_adjusted_metrics(client: bigquery.Client, min_games: int = 20) -> pd.DataFrame:
    ...
    WITH player_season_stats AS (
      ...
      JOIN `fantasy-snipe-ai.nhl_raw.players` p ON p.id = pgm.player_id
      WHERE pgm.game_type = 2  -- Regular season only
      GROUP BY p.id, p.full_name, p.position, pgm.season, pgm.game_type
...
def create_season_analysis(df: pd.DataFrame, metric: str, title: str) -> None:
    ...
    for tier in ['Elite', 'High', 'Middle']:
        ...
        if len(tier_data) > 0:
            ...
            ax3.plot(season_curve['season'], season_curve['mean'], 
    ...
    ax4 = axes[1, 1]
    season_counts = plot_df.groupby(['season', 'performance_tier']).size().reset_index(name='count')
    for tier in ['Elite', 'High', 'Middle', 'Lower']:
...
def analyze_performance_tiers(df: pd.DataFrame) -> Dict:
    """Analyze performance characteristics by tier and position."""
...
def main():
    ...
    print(elite_indicators[['full_name', 'season', 'pts60_percentile', 'cf_pct_percentile', 'toi_zscore']].head(20))
