...
def get_pts60_age_data(client: bigquery.Client, min_games: int = 20) -> pd.DataFrame:
    """Get PTS/60 data with age calculations."""
...
def create_pts60_age_curves(df: pd.DataFrame) -> None:
    """Create focused PTS/60 age curve visualizations."""
    
    # Filter out goalies for PTS/60 analysis
    plot_df = df[df['position_group'] != 'Goalie'].copy()
    
    # Create the plot
    fig, axes = plt.subplots(2, 2, figsize=(16, 12))
    fig.suptitle('PTS/60 Age Curves by Performance Tier', fontsize=16, fontweight='bold')
    ...
    ax1.set_title('PTS/60 by Age - All Performance Tiers')
    ax1.legend()
    ...
    ax2.set_title('Elite vs High Tier PTS/60 Comparison')
    ax2.legend()
    ...
    ax3.set_title('Forwards Only - PTS/60 by Age')
    ax3.legend()
...
def identify_early_elite_pts60(df: pd.DataFrame) -> pd.DataFrame:
    """Identify young players showing elite PTS/60 trajectory indicators."""
...
def analyze_pts60_peak_ages(df: pd.DataFrame) -> Dict:
    """Analyze peak PTS/60 ages by tier and position."""
...
def main():
    parser = argparse.ArgumentParser(description='Analyze PTS/60 age curves')
    parser.add_argument('--min-games', type=int, default=20, help='Minimum games played per season')
    parser.add_argument('--save-plots', action='store_true', help='Save plots to files')
    args = parser.parse_args()
    
    print("Loading PTS/60 age data...")
    client = bigquery.Client()
    df = get_pts60_age_data(client, args.min_games)
    ...
    print("\nCreating PTS/60 age curves...")
    create_pts60_age_curves(df)
    
    if args.save_plots:
        plt.savefig('pts60_age_curves.png', dpi=300, bbox_inches='tight')
        print("Saved plot: pts60_age_curves.png")
    
    # Identify early elite indicators
    print("\nIdentifying early elite PTS/60 indicators...")
    early_elite = identify_early_elite_pts60(df)
    
    print(f"\nFound {len(early_elite)} young players with elite PTS/60 indicators:")
    print(early_elite[['full_name', 'age', 'season', 'avg_pts60', 'pts60_percentile', 'position']].head(20))
    
    # Analyze peak ages
    print("\nAnalyzing peak PTS/60 ages...")
    peak_analysis = analyze_pts60_peak_ages(df)
    
    print("\nPeak PTS/60 Age Analysis:")
    ...
    print("\nPTS/60 age curve analysis complete!")
