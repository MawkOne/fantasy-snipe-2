#!/usr/bin/env python3
"""
Edmonton Oilers On-Ice vs Off-Ice Impact Analysis - 2024-25 Season
Analyze each Oiler's relative impact when on ice vs off ice
"""
...
def get_oilers_2024_25_data() -> pd.DataFrame:
    """
    Get 2024-25 season data for Edmonton Oilers players
    """
    client = bigquery.Client()
    
    # Query to get 2024-25 season data for Oilers players - 5v5 only
    query = """
    SELECT 
        ps.full_name,
    ...
    AND ps.position != 'G'
    AND ps.team_abbrev = 'EDM'
    ORDER BY ps.ev_points DESC
    """
    
    return client.query(query).to_dataframe()

def calculate_oilers_team_totals_and_relative_impact(df: pd.DataFrame) -> pd.DataFrame:
    """
    Calculate Oilers team totals and each player's relative contribution
    """
    df = df.copy()
    
    # Calculate Oilers team totals
    team_totals = df.agg({
        'points': 'sum',
        'goals': 'sum',
        'assists': 'sum',
...
def classify_oilers_players(df: pd.DataFrame) -> pd.DataFrame:
    """
    Classify Oilers players into tiers
    """
    df = df.copy()
...
def create_oilers_on_off_ice_visualization(df: pd.DataFrame) -> None:
    """
    Create visualization for Oilers on-ice vs off-ice impact
    """
    # Create the plot
    ...
    ax1.set_title('Edmonton Oilers: On-Ice vs Off-Ice Points Impact (5v5 Only)\\n2024-25 Season')
    ax1.set_xticks(x)
    ...
    ax2.set_title('Edmonton Oilers: On-Off Ice Impact Score (5v5 Only)\\n2024-25 Season')
    ...
    ax3.set_title('Edmonton Oilers: Relative Team Impact Score (5v5 Only)\\n2024-25 Season')
    ...
    ax4.set_title('Edmonton Oilers: Points/60 vs TOI per Game (5v5 Only)\\n2024-25 Season')
...
def create_detailed_oilers_analysis_table(df: pd.DataFrame) -> None:
    """
    Create detailed table of Oilers on-ice vs off-ice analysis
    """
    print("\\n" + "="*150)
    print("EDMONTON OILERS ON-ICE vs OFF-ICE IMPACT ANALYSIS - 2024-25 SEASON (5v5 ONLY)")
    print("="*150)
    ...
    # Key insights
    print("\\n" + "="*150)
    print("KEY INSIGHTS - EDMONTON OILERS")
    print("="*150)
    ...
    # Team totals
    print(f"\\n📊 EDMONTON OILERS TEAM TOTALS - 2024-25 (5v5 ONLY):")
...
def main():
    """
    Main function to run the Oilers on-ice vs off-ice impact analysis
    """
    print("EDMONTON OILERS ON-ICE vs OFF-ICE IMPACT ANALYSIS - 2024-25 SEASON (5v5 ONLY)")
    print("Analyzing each Oiler's relative impact when on ice vs off ice - Even Strength Only")
    print("="*80)
    
    # Get Oilers 2024-25 player data
    print("\\nFetching Edmonton Oilers 2024-25 season data...")
    try:
        oilers_df = get_oilers_2024_25_data()
        print(f"✅ Found data for {len(oilers_df)} Edmonton Oilers players in 2024-25 season")
    except Exception as e:
        print(f"❌ Error fetching Oilers data: {e}")
        return
    
    # Calculate team totals and relative impact
    print("\\nCalculating team totals and relative impact...")
    df_with_team_totals = calculate_oilers_team_totals_and_relative_impact(oilers_df)
    
    # Classify players
    print("\\nClassifying Oilers players into tiers...")
    classified_df = classify_oilers_players(df_with_team_totals)
    
    # Create visualizations
    print("\\nCreating Oilers on-ice vs off-ice impact visualizations...")
    create_oilers_on_off_ice_visualization(classified_df)
    
    # Create detailed analysis
    create_detailed_oilers_analysis_table(classified_df)
    
    print("\\n" + "="*80)
    print("EDMONTON OILERS ANALYSIS COMPLETE")
    print("="*80)
    print("\\nThis analysis shows each Oiler's impact when they are on the ice versus when they are off the ice (5v5 only).")
    ...
    print("- This metric helps identify the most important players to the Oilers' success")
