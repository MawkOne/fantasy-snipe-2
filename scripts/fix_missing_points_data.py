...
def fix_missing_points_data():
    """Fix the analysis to handle missing points data for players like Pastrnak"""
    
    client = bigquery.Client()
    
    print("="*80)
    print("FIXING MISSING POINTS DATA ISSUE")
    print("="*80)
    
    # Get comprehensive player analysis with better handling of missing data
    comprehensive_query = """
    ...
    position_percentiles AS (
        ...
        WHERE points > 0  -- Only use players with actual points data for percentiles
        GROUP BY position
    ),
    matched_players AS (
        SELECT 
            ...
            CASE 
                ...
                ELSE "Aging"
            END as age_category,
            -- Performance tiers with special handling for missing points data
            CASE 
            ...
            -- Future elite potential (young players with high production)
            CASE 
                WHEN ps.current_age <= 25 AND ps.points_60 >= pos.p80_points_60 AND (ps.points >= pos.p80_total_points OR ps.points = 0) THEN "Future Elite"
    ...
    deduplicated_players AS (
        SELECT 
            ...
            ROW_NUMBER() OVER (PARTITION BY team_abbr, player_name ORDER BY points_60 DESC) as rn
    ...
    # Check specifically for Pastrnak
    pastrnak_data = player_results[player_results['player_name'].str.contains('Pastrnak', na=False)]
    if not pastrnak_data.empty:
        print("Pastrnak Classification:")
        for _, row in pastrnak_data.iterrows():
            print(f"Team: {row['team_abbr']}")
            print(f"Name: {row['player_name']}")
            print(f"Age: {row['current_age']} ({row['age_category']})")
            print(f"TOI: {row['toi_per_game']:.1f}")
            print(f"Pts/60: {row['points_60']:.1f}")
            print(f"Points: {row['points']}")
            print(f"Performance Tier: {row['performance_tier']}")
            print(f"Future Elite: {row['future_elite_potential']}")
    else:
        print("Pastrnak still not found in results")
    ...
    with open('/Users/markhenderson/Cursor Projects/NHL-API/projected_rosters_2025_26.md', 'w') as f:
        f.write('# 2025-26 Projected NHL Rosters - Age-Based Analysis (Fixed)\n\n')
        f.write('**FINAL VERSION** - Fixed missing points data issue for players like Pastrnak\n\n')
        f.write('## Elite Player Definition:\n')
        f.write('- **Forwards (C, L, R)**: Top 5% in Pts/60 AND (Top 10% in total points OR missing points data)\n')
        f.write('- **Defensemen (D)**: Top 5% in Pts/60 AND (Top 10% in total points OR missing points data)\n\n')
        
        f.write('## Player Categories:\n')
        f.write('- **Elite**: Top 5% Pts/60 + (Top 10% total points OR missing points data)\n')
        f.write('- **Future Elite**: Age ≤25 + Top 20% Pts/60 + (Top 20% total points OR missing points data)\n')
        f.write('- **Near Elite**: Top 10% Pts/60 + (Top 20% total points OR missing points data)\n')
        f.write('- **Good**: Top 20% Pts/60 + (Top 20% total points OR missing points data)\n')
        f.write('- **Core Player**: 18+ minutes TOI per game\n\n')
        
        f.write('## Age Categories:\n')
        f.write('- **Young**: ≤22 years old\n')
        f.write('- **Rising**: 23-27 years old (pre-peak)\n')
        f.write('- **Peak**: 28-32 years old (prime)\n')
        f.write('- **Veteran**: 33-35 years old (post-peak)\n')
        f.write('- **Aging**: 36+ years old (declining)\n\n')
    ...
    print("✅ Fixed missing points data issue!")
