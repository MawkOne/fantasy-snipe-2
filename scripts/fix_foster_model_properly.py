#!/usr/bin/env python3
"""
Fix the Foster model properly:
1. Ensure each player appears only once
2. Use realistic TOI per player (18-22 minutes for top players)
3. Fix the line assignment logic
"""
...
def fix_foster_model_properly():
    """Fix the Foster model properly with realistic TOI and no duplicates."""
    client = bigquery.Client()
    
    print("Fixing Foster model properly...")
    
    # 1. Create proper line assignments (one per player, realistic TOI)
    print("1. Creating proper line assignments...")
    line_assignments_query = """
    CREATE OR REPLACE TABLE `fantasy-snipe-ai.nhl_projections.line_assignments_proper` AS
    ...
    line_assignments AS (
        SELECT 
            ...
            -- REALISTIC TOI per player (not per line)
            CASE 
                WHEN tr.position_group = 'F' AND tr.team_position_rank <= 3 THEN 20.0  -- 1st line forwards
                WHEN tr.position_group = 'F' AND tr.team_position_rank <= 6 THEN 18.0  -- 2nd line forwards
                WHEN tr.position_group = 'F' AND tr.team_position_rank <= 9 THEN 16.0  -- 3rd line forwards
                WHEN tr.position_group = 'F' AND tr.team_position_rank <= 12 THEN 14.0 -- 4th line forwards
                WHEN tr.position_group = 'D' AND tr.team_position_rank <= 2 THEN 25.0  -- 1st pair defense
                WHEN tr.position_group = 'D' AND tr.team_position_rank <= 4 THEN 22.0  -- 2nd pair defense
                WHEN tr.position_group = 'D' AND tr.team_position_rank <= 6 THEN 18.0  -- 3rd pair defense
                ELSE 12.0  -- Depth players
            END as forecast_toi_minutes,
            
            CURRENT_TIMESTAMP() as created_at,
            'Foster Model v1.5 (Proper)' as model_version
    ...
    job.result()
    print("✓ Created proper line assignments (one per player, realistic TOI)")
    
    # 2. Create proper line forecasts
    print("2. Creating proper line forecasts...")
    line_forecasts_query = """
    CREATE OR REPLACE TABLE `fantasy-snipe-ai.nhl_projections.line_forecasts_proper` AS
    WITH team_lines AS (
        SELECT 
            la.team,
            la.primary_role,
        ...
        FROM `fantasy-snipe-ai.nhl_projections.line_assignments_proper` la
    ...
    job.result()
    print("✓ Created proper line forecasts")
    
    # 3. Create proper player forecasts
    print("3. Creating proper player forecasts...")
    player_forecasts_query = """
    CREATE OR REPLACE TABLE `fantasy-snipe-ai.nhl_projections.player_forecasts_proper` AS
    WITH player_line_forecasts AS (
        ...
        FROM `fantasy-snipe-ai.nhl_projections.line_assignments_proper` la
        LEFT JOIN `fantasy-snipe-ai.nhl_projections.line_forecasts_proper` lf 
    ...
    job.result()
    print("✓ Created proper player forecasts")
    
    # 4. Create final proper view
    print("4. Creating final proper view...")
    final_view_query = """
    CREATE OR REPLACE VIEW `fantasy-snipe-ai.nhl_projections.current_player_forecasts_final_proper` AS
    SELECT 
        pf.player_id,
        pf.player_name,
        pf.position,
        ...
        pf.created_at,
        pf.model_version
    FROM `fantasy-snipe-ai.nhl_projections.player_forecasts_proper` pf
    ORDER BY pf.forecast_points_60_adjusted DESC
    """
    
    job = client.query(final_view_query)
    job.result()
    print("✓ Created final proper view")
    
    # Run comprehensive QA
    print("\nRunning comprehensive QA on proper model...")
    ...
    FROM `fantasy-snipe-ai.nhl_projections.line_assignments_proper`
    ...
    for row in qa_results2:
        print(f"  {row.primary_role}: {row.player_count} players, {row.avg_toi_per_player} min avg ({row.min_toi}-{row.max_toi})")
    ...
    FROM `fantasy-snipe-ai.nhl_projections.current_player_forecasts_final_proper`
    WHERE team = 'EDM'
