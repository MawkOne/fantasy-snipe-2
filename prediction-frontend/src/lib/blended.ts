...
export type SourceRank = { source: string; rank: number }
export type PlayerEntry = { name: string; ranks: SourceRank[]; blended: number }
...
function choosePointsColumn(headers: string[]): number {
  const preferred = [/^(proj\s*)?points?$/i, /^pts$/i, /fantasy\s*points/i, /^fp$/i, /total\s*points/i, /proj/i]
...
async function loadDirectoryAsRanking(dirPath: string): Promise<Map<string, number>> {
  ...
  try {
    ...
    files = items.filter((i) => i.isFile()).map((i) => path.join(dirPath, i.name))
  } catch {
    return new Map()
  }
