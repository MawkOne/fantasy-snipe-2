...
export function PlayerProjectionRechart({
  ...
  // For ALL we lock to the full-season projection scale; for Last 10 / YTD we scale to the max within the selected window
  const currentMaxInWindow = data.reduce((mx, d) => (d.current !== undefined && d.current > mx ? d.current : mx), 0)
  const projectionAtWindowEnd = (projectionTotal * (timeframe === "ALL" ? lastGame : endGame)) / lastGame
  const baseMax = timeframe === "ALL" ? projectionTotal : Math.max(projectionAtWindowEnd, currentMaxInWindow)
